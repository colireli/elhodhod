...
@php
    ...
    if($user_role == $auth_branch){
        ...
        $branches = Modules\Cargo\Entities\Branch::where('is_archived', 0)->where('id', $userClient->branch_id)->get();
    }
...
<div class="row">
    <div class="col-lg-12">
        ...
        <div class="form-group row">
            <label class="col-form-label fw-bold fs-6 required">{{ __('shipment delivery type') }}</label>
            <div class="col-9 col-form-label">
                <div class="radio-inline">
                    <label class="radio radio-success" style="margin-right: 20px;">
                        <input @if(Modules\Cargo\Entities\ShipmentSetting::getVal('def_shipment_type')=='1' ) checked @endif type="radio" name="Shipment[delivery_type]" checked="checked" value="1"
                            {{ old('Shipment.delivery_type') == 1 ? 'checked' : '' }}
                            @if($typeForm == 'edit')
                                {{ $model->delivery_type == 1 ? 'checked' : '' }}
                            @endif
                        />
                        <span></span>
                        {{ __('HOME') }}
                    ...
                    <label class="radio radio-success ">
                        <input @if(Modules\Cargo\Entities\ShipmentSetting::getVal('def_shipment_type')=='2' ) checked @endif type="radio" name="Shipment[delivery_type]" value="2"
                            {{ old('Shipment.delivery_type') == 2 ? 'checked' : '' }}
                            @if($typeForm == 'edit')
                                {{ $model->delivery_type == 2 ? 'checked' : '' }}
                            @endif
                        />
                        <span></span>
                        {{ __('DESK') }}
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group client-select">
                    @if($user_role == $auth_client)
                        <input type="hidden" placeholder="" class="form-control" name="" value="{{$userClient->name}}" disabled>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    @if($user_role == $auth_client)
                        @if($typeForm == 'create')
                            <input type="tel" name="Shipment[client_phone]" placeholder="{{ __('cargo::view.client_phone') }}"  id="client_phone" value="{{$userClient->country_code.($userClient->responsible_mobile ??  base_country_code())}}" class="d-none number-only form-control @error('Shipment.client_phone') is-invalid @enderror" />
                        ...
                        @elseif($typeForm == 'edit')
                            <input type="tel" name="Shipment[client_phone]" placeholder="{{ __('cargo::view.client_phone') }}"  id="client_phone" value="{{ old('Shipment.client_phone', isset($model) ? $model->client_phone :  base_country_code() ) }}" class="d-none number-only  form-control @error('Shipment.client_phone') is-invalid @enderror" />
                    ...
                    @else
                        ...
                        <input type="tel" name="Shipment[client_phone]" placeholder="{{ __('cargo::view.client_phone') }}"  id="client_phone" value="{{ old('Shipment.client_phone', isset($model) ? $model->client_phone :  base_country_code() ) }}" class="phone_input phone_input_1  number-only  form-control @error('Shipment.client_phone') is-invalid @enderror" />
                        <input type="hidden" class="country_code country_code_1" id="country_code" name="Shipment[country_code]" value="{{ old('country_code', isset($model) ? $model->country_code :  base_country_code()) }}" data-reflection="phone">
                    @endif
            ...
            <div class="col-md-6">
                <div class="form-group client-select">
                    ...
                    @if($user_role == $auth_client)
                        <input type="text" placeholder="" class="form-control" name="Shipment[reciver_name]" value="{{ old('Shipment.reciver_name', isset($model) ? $model->reciver_name : '') }}" >
                    @else
                        <select
                            id="reciver-id"
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    @if($user_role == $auth_client)
                        ...
                        @elseif($typeForm == 'edit')
                            <input type="tel" name="Shipment[reciver_phone]" placeholder="{{ __('cargo::view.reciver_phone') }}"  id="reciver_phone" value="{{ old('Shipment.reciver_phone', isset($model) ? $model->reciver_phone : '') }}" class="phone_input phone_input_2  number-only  form-control @error('Shipment.reciver_phone') is-invalid @enderror" />
                            <input type="hidden" class="country_code country_code_2" id="country_code" name="Shipment[country_code]" value="" data-reflection="phone">
                        @endif
                    @else
                        <input type="tel" name="Shipment[reciver_phone]" placeholder="{{ __('cargo::view.reciver_phone') }}"  id="reciver_phone" value="{{ old('Shipment.reciver_phone', isset($model) ? $model->reciver_phone :  base_country_code() ) }}" class="phone_input phone_input_2  number-only  form-control @error('Shipment.reciver_phone') is-invalid @enderror" />
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="text-left modal-body">
                            <div class="row">
                                <div class="col-6">{{ __('Delivery Type') }} :</div>
                                <div class="col-6" id="delivery_type_"></div>
                            </div>
                            <div class="row">
                                @if($is_def_mile_or_fees=='2')
...
@push('js-component')
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.8/js/utils.js" crossorigin="anonymous"></script>
    ...
    <script>
        ...
        function getClientAddresses(client_id)
        {
            ...
            $.get("{{route('ajax-get-client-addresses-ajax')}}?client_id=" + id, function(data) {
                if(data.length != 0){
                    ...
                    $.get("{{route('client.get.one.address')}}?address_id=" + data[0]['id'], function(data) {
                        $("#change-country").val(data[0]['country_id']).change();
                        $("#change-country-to").val(data[0]['country_id']).change();
                        setTimeout(function(){
        ...
        function changeCountryTo(id){
            $.get("{{route('ajax.getStates')}}?country_id=" + id, function(data) {
                ...
                for (let index = 0; index < data.length; index++) {
                    ...
                    var edit_to_state_id = {{isset($model)? $model->to_state_id : 'null' }}
                    if(edit_to_state_id == element['id'] ){
        ...
        function select_areas(id){
            var delivery_type = $("input[type='radio'][name='Shipment[delivery_type]']:checked").val();
            if(delivery_type == 2){
                getToAreas();
            }else{
                changeStateTo(id);
        ...
        $('#client-id').change(function() {
            var id = document.getElementsByName("Shipment[to_state_id]")[0].value;
            select_areas(id);
        });

        $("input[type='radio'][name='Shipment[delivery_type]']").change(function() {
            var id = document.getElementsByName("Shipment[to_state_id]")[0].value;
            select_areas(id);
        });
        ...
        function get_estimation_cost() {
            ...
            var amount_to_be_collected = document.getElementsByName("Shipment[amount_to_be_collected]")[0].value;
            ...
            $.post('{{ route('shipments.get-estimation-cost') }}', request_data, function(response){
                ...
                document.getElementById("delivery_type_").innerHTML = response.delivery_type;
