...
@section('content')
                ...
                <div class="col-md-10">
                    <div class="pb-10 d-flex justify-content-between pb-md-20 flex-column flex-md-row">
                        <div class="px-0 d-flex flex-column align-items-md-start">
                            <span class="d-flex flex-column align-items-md-start">
                                ...
                                @endif
                                @if($shipment->track != null && ($user_role == 1 || $user_role == 3 || auth()->user()->can('create-branches')))
                                    <span><span class="font-weight-bolder opacity-70">{{ __('TRACKING') }}:</span> {{$shipment->track}}</span>
                                    <!-- <span class="mb-5 font-weight-bolder"><?=$d->getBarcodeHTML($shipment->track, "C128");?></span> -->
                                @endif
                            </span>
                        </div>
                    ...
                    <div class="pb-6 d-flex justify-content-between">
                        ...
                        <div class="d-flex flex-column flex-root">
                            ...
                            <span class="opacity-70 d-block">{{$shipment->statut_activity}}</span>
                    ...
                    <div class="pt-6 d-flex justify-content-between">
                        ...
                        <div class="d-flex flex-column flex-root">
                            <span class="mb-2 font-weight-bolder">{{ __('shipment delivery type') }}</span>
                            <span class="opacity-70">{{($shipment->delivery_type == 1)?'HOME':'DESK'}}</span>
                        </div>
                        <div class="d-flex flex-column flex-root">
                    ...
                    <div class="pt-6 d-flex justify-content-between">
                        ...
                        @if(isset($shipment->from_area))
                            <div class="d-flex flex-column flex-root">
                                ...
                                <span class="text-muted font-weight-bolder font-size-lg">{{$shipment->from_area->name}}</span>
                            </div>
                        @endif
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr class="font-weight-bolder">
                                    <td>{{$shipment->payment_method_id}} ({{__('cargo::view.payment_type') .":". $shipment->getPaymentType()}})</td>
                                    ...
                                    <td class="text-right text-primary font-size-h3 font-weight-boldest">{{format_price($shipment->tax + $shipment->shipping_cost + (($shipment->insurance * $shipment->amount_to_be_collected) / 100)) }}<br /><span class="text-muted font-weight-bolder font-size-lg">{{ __('cargo::view.included_tax_insurance') }}</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="px-8 py-8 row justify-content-center py-md-10 px-md-0">
                <div class="col-md-10">
                    <div class="d-flex justify-content-between">
                        ...
                        @endif
                        @if($shipment->status_id != Modules\Cargo\Entities\Shipment::DRAFT_STATUS)
                        ...
                        <a href="{{route('shipments.print', array($shipment->id, 'tracking'))}}" class="btn btn-light-primary font-weight-bold" target="_blank">{{ __('Print Tracking') }}<i class="ml-2 la la-file-invoice"></i></a>
                        @endif
                        @if($user_role == $admin || auth()->user()->can('edit-shipments' || ($user_role == $client && $shipment->status_id == Modules\Cargo\Entities\Shipment::DRAFT_STATUS)))
    ...
    <!--end::List Widget 19-->
    @if(($user_role == $admin || $user_role == $branch || auth()->user()->can('shipments-log')) && (!empty($shipment->logs->toArray()) || $shipment->activity))
        <div class="card card-custom card-stretch-half gutter-b">
            <!--begin::List Widget 19-->
            ...
            <div class="pt-2 card-body" style="overflow:hidden">
                ...
                @if($shipment->activity)
                 <!--begin::Item-->
                    <div class="timeline-item align-items-start">
                            <!--begin::Label-->
                            <div class="timeline-label font-weight-bolder text-dark-75 font-size-lg">{{ __('log') }} </div>
                            <!--end::Label-->
                            <!--begin::Badge-->
                            <div class="timeline-badge">
                                <i class="fa fa-genderless text-warning icon-xl" style="margin-right: 4px;"></i>
