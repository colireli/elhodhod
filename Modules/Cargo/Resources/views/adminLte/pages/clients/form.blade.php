...
<!--begin::Input group --  Subcontracting  -->
<div class="row mb-6">

    <div class="col-lg-12 card-header">
        <h5>{{ __('Subcontracting') }}</h5>
    </div>

    <!--begin::Input group-->
    <div class="fv-row col-lg-12 form-group">
    <!--begin::Label-->
        <label class="col-form-label fw-bold fs-6 required">{{ __('Costumer Plan') }}</label>
    <!--end::Label-->
        <!--end::Label-->
        <div class="input-group mb-4">
            <select
                ...
                id="plan_id"
                data-control="select2"
                data-placeholder="{{ __('choose plan') }}"
                data-allow-clear="true"
            >
            </select>
            @error('plan_id')
...
function getBranchPlan(id){
    $.get("{{route('admin.plan.get-branch-plan')}}?id=" + id, function(data) {
            ...
            // $('select[name="plan_id"]').append('<option value="">Select Plan</option>');
            var edit_plan_id = <?php echo e(isset($model) ? $model->plan_id : 'null'); ?>;

            for (let index = 0; index < data.length; index++) {
                const element = data[index];
                if(edit_plan_id == element['id']){
                    $('select[name ="plan_id"]').append('<option value="' + element['id'] + '"  selected >' + element['title'] + '</option>');

                }else{

                    $('select[name ="plan_id"]').append('<option value="' + element['id'] + '" >' + element['title'] + '</option>');
                }
            }
    });
}
