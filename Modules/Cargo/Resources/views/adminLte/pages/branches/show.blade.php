...
@section('content')
    ...
    @include('cargo::adminLte.pages.branches.overview-profile', ['model' => $model, 'shipments' => $shipments])
    ...
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="card-header">
            ...
            <div class="card-title m-0">
                ...
                <h4><a href="@if(auth()->check() && auth()->user()->role == 1)
                                    {{ route('branchs.login', encrypt($model->id)) }}
                                    @else
                                    #
                                    @endif" class="text-dark-50 text-hover-primary font-weight-bold mr-lg-8 mr-5 mb-lg-0 mb-2">
                                     <i class="la la-laptop mr-2 ms-2 font-size-lg"> </i>{{ $model->name}}</a></h4>
            </div>
            <!--end::Card title-->

            @if(auth()->user()->can('edit-branches') || $user_role == $admin)
                <a href="{{ fr_route('branches.edit', ['branch' => $model->id]) }}" class="btn btn-primary align-self-center">{{ __('view.edit_profile') }}</a>
            @endif
