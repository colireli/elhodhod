...
<!-- begin: Btn Edit Row -->
@if(auth()->user()->can('edit-stopdesks') || $user_role == $admin)
    <a
        href="{{ fr_route('stopdesks.edit', $model->id) }}"
...
@if(auth()->user()->can('delete-stopdesks') || $user_role == $admin)
    <button
        type="button"
        data-action="{{ fr_route('stopdesks.destroy', $model->id) }}"
