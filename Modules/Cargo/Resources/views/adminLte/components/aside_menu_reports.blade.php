@php
    ...
    if($user_role == 4){
        ...
        $client_id = Modules\Cargo\Entities\Client::where('user_id',auth()->user()->id)->first()->id;
    }
    $admin = 1;
    $staff = 0;
    $branch = 3;
...
@if (in_array($user_role, [$client]) )
    <li class="nav-item  {{ areActiveRoutes(['shipments.rappayment.client_payments'], 'menu-is-opening menu-open active') }}">
        <a href="{{ fr_route('shipments.rappayment.client_payments', $client_id )}}"
            class="nav-link {{ areActiveRoutes(['shipments.rappayment.client_payments'], 'menu-is-opening menu-open active') }}">
            <i class="fas fa-file"></i>
            <p>{{ __('Payment Report') }}</p>
        </a>
    </li>
@endif
